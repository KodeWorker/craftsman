...
def mock_auth(mocker):
    ...
    instance.USERNAME_LIST = ["LLM_BASE_URL", "LLM_API_KEY", "LLM_SSL_CRT"]
...
def test_init_creates_three_directories(runner, mock_config, mock_makedirs):
    result = runner.invoke(main, ["init"])
    assert result.exit_code == 0
    assert mock_makedirs.call_count == 3


def test_init_outputs_root_path(runner, mock_config, mock_makedirs):
    result = runner.invoke(main, ["init"])
    assert "/tmp/craftsman-test" in result.output


def test_init_exits_zero(runner, mock_config, mock_makedirs):
...
def test_auth_list_shows_set_value(runner, mock_auth):
    mock_auth.get_password.side_effect = lambda k: (
        "myval" if k == "LLM_API_KEY" else ""
    )
...
def test_auth_list_shows_not_set(runner, mock_auth):
    mock_auth.get_password.return_value = ""
    result = runner.invoke(main, ["auth", "list"])
...
def test_auth_get_shows_value(runner, mock_auth):
